...
export const OrderRow = ({ order, imageUrl }) => {
    ...
    const handleStatusChange = async (event) => {
        ...
        try {
            const response = await updateOrderStatus(order._id, newStatus); // Assuming updateOrderStatus handles API call

            if (response.success) {
                setStatus(newStatus); // Update local state with new status
            } else {
                console.error("Failed to update order status:", response.message);
                setStatus(order.status); // Revert local state if update fails
            }
        } catch (error) {
            console.error("Failed to update order status:", error);
            setStatus(order.status); // Revert local state if update throws an error
        }
    };
