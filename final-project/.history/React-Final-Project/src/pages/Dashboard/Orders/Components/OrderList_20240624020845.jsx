...
const OrderList = () => {
  ...
  // Helper function to find the product image URL based on the order product ID
  // const getProductImageUrl = (productId) => {
  //   const product = products?.products?.find(p => p._id === productId);
  //   return product ? product.imagesUrl.images[0] : "";
  // };
    ...
    >
      ...
      {orders?.orders?.map((order, index) => (
        <OrderRow 
          key={index} 
          order={order} 
          // imageUrl={getProductImageUrl(products.products.map((product)=>{
          //   return product._id
          // }))} // Pass the correct image URL to OrderRow
        />
      ))}
    </Grid>
  );
