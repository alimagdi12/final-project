  ...
  <AppBar position="static" sx={{ background: color, zIndex: 9 }}>
    <Container maxWidth="xl">
      <Toolbar disableGutters>
        ...
        >
          {['Products', 'About Us', 'Posts'].map((text, index) => (
            <Box sx={{ my: 2, textAlign: "center", position: "relative" }} key={index}>
              <Link to={`/${text.toLowerCase().replace(' ', '')}`} className="text-decoration-none h5 mx-2">
                {text}
              </Link>
            </Box>
          ))}
          ...
          >
            ...
            {hoveredPage && (
              ...
              >
                <Grid item xs={12} md={6} sx={{ zIndex: "999", height: "100%", maxWidth: '100%' }}>
                  {categories && categories.categories && categories.categories.map((category) => (
